...
export function OperationsMap({ reports = [], routes = [], checkpoints = [], onSelect }) {
  ...
  const checkpointPoints = checkpoints.filter(item => Number.isFinite(Number(item.latitude)) && Number.isFinite(Number(item.longitude))).map(item => ({ checkpoint: item, coordinates: [Number(item.latitude), Number(item.longitude)] })); const bounds = [...reportPoints.map(item => item.coordinates), ...routesWithLines.flatMap(item => item.coordinates), ...checkpointPoints.map(item => item.coordinates)];
  return <section className="map-panel"><div className="map-title"><span><MapRegular/> Live operations map</span><small>Internal SafeRide map</small></div><div className="leaflet-map"><MapContainer center={center} zoom={12} scrollWheelZoom>{routesWithLines.map(({ route, coordinates }) => <Polyline key={route.route_id} positions={coordinates} pathOptions={{ color: '#397c62', weight: 4, opacity: .8 }}/>) }{checkpointPoints.map(({ checkpoint, coordinates }) => <Marker key={checkpoint.checkpoint_id} position={coordinates}><Popup><strong>{checkpoint.name}</strong><br/>{checkpoint.is_active ? 'Active checkpoint' : 'Inactive checkpoint'}</Popup></Marker>)}{reportPoints.map(({ report, coordinates }) => <Marker key={report.case_id} position={coordinates} icon={reportIcon(report.status)}><Popup><strong>{report.case_id}</strong><br/>{report.route_name}<br/><StatusBadge value={report.status}/><br/><Button size="small" appearance="primary" onClick={() => onSelect?.(report.case_id)}>Open case</Button></Popup></Marker>)}<Bounds points={bounds}/></MapContainer></div></section>;
}
